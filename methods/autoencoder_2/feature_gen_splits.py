...
device = "cpu:0"
saved = torch.load(split_folder + "/models/autoencoder_2_s" + str(split), map_location='cpu')
...
for inp_full_name in test_files:
    ...
    df = pd.DataFrame({'autoencoder_2_pred': out_scores})

    fname = split_folder + '/features/train/autoencoder_2_' + inp_full_name[len(split_folder + "/test/log_"):]
    df[['autoencoder_2_pred']].to_pickle(fname)
