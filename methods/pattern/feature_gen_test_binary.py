...
skip_patterns = np.load(split_folder + '/models/skip_means_binary.npy')
skip_nums = np.load(split_folder + '/models/skip_nums_binary.npy')
...
for(inp_full_name, pre_full_name) in zip(inp_files, pre_files):
    ...
    pre_skip_vals = pre_full.skip.values >= 2
    ...
    for i in range(0, 11):
        powersequences[i] = 2**np.flip(np.arange(i))
    ...
    inp_full['pred_binary_proba'] = preds
    inp_full['pred_binary_confidence'] = preds_confidence

    fname = split_folder + '/features/test/binary_pattern_' + inp_full_name[len(split_folder + "/test/log_input_"):]
    inp_full[['pred_binary_proba', 'pred_binary_confidence']].to_pickle(fname)
