...
device = "cpu:0"
saved = torch.load(split_folder + "/models/autoencoder_1_s" + str(split), map_location='cpu')
...
for inp_full_name in test_files:
    print(inp_full_name)
    ...
    inp_skip_values = (inp_full['skip'].values >= 2) * 2 - 1
    ...
    inp_track_values_split = np.split(inp_track_values * inp_skip_values, inp_split_indices)

    correct_order = [i for t in zip(inp_track_values_split, zero_pads) for i in t]
    ...
    df = pd.DataFrame({'autoencoder_1_pred': out_scores})

    fname = split_folder + '/features/train/autoencoder_1_' + inp_full_name[len(split_folder + "/test/log_"):]
    df[['autoencoder_1_pred']].to_pickle(fname)
